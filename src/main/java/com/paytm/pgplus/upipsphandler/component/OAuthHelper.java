...
import com.paytm.pgplus.upipsphandler.service.IOAuthHelper;
...
//                configurationDataService.getPaytmProperty(TheiaConstant.ExtraConstants.OAUTH_CLIENT_ID).getValue(),
//                configurationDataService.getPaytmProperty(TheiaConstant.ExtraConstants.OAUTH_CLIENT_SECRET_KEY).getValue(),
//                body.getMid());
//        try {
...
//    }
    ...
    public UserDetailsBiz validateSSOToken(InitiateTransactionRequestBody body)
        ...
        if (StringUtils.isBlank(body.getPaytmSsoToken())) {
            LOGGER.error("Token provided is blank");
            throw PaymentRequestProcessingException.getException(String.valueOf(ResultCode.REQUEST_PARAMS_VALIDATION_EXCEPTION));
        }
        ...
        try {
            ...
            if (!fetchUserDetailsResponse.isSuccessfullyProcessed()
                    || (Objects.isNull(fetchUserDetailsResponse.getUserDetails()))) {
                LOGGER.error("User details fetching failed or UserDetails is null");
                throw PaymentRequestProcessingException.getException(String.valueOf(ResultCode.INVALID_SSO_TOKEN));
            }
            LOGGER.debug("User authentication successfully");
