...
import com.paytm.pgplus.upipsphandler.model.PayMethod;
...
import com.paytm.pgplus.upipsphandler.model.SubscriptionDetail;
...
import com.paytm.pgplus.upipsphandler.enums.PaymodeSequenceEnum;
import com.paytm.pgplus.upipsphandler.enums.TokenType;
import com.paytm.pgplus.upipsphandler.enums.common.InstrumentType;
...
import com.paytm.pgplus.upipsphandler.model.common.NativePersistData;
import com.paytm.pgplus.upipsphandler.model.common.UserAgentInfo;
...
import com.paytm.pgplus.upipsphandler.constants.TheiaConstant.ExtraConstants;
...
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.MerchantPreference.PreferenceKeys.*;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.PrepaidCard.PREPAID_CARD_MAX_AMOUNT;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.LINK_BASED_KEY;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.Native.CHECKOUT;
import static com.paytm.pgplus.upipsphandler.service.impl.AccessTokenUtils.getTokenExpiryTime;
...
public class NativePayviewConsultRequestProcessor
    ...
    private void disableUPICollectIfMerchantNotVerified(InitiateTransactionRequestBody orderDetail,
        ...
        try {
            ...
            Double upiCollectLimitUnverified = Double.parseDouble(ConfigurationUtil
                    .getProperty(UPI_COLLECT_LIMIT_UNVERIFIED, "2000"));
            if (orderAmount > upiCollectLimitUnverified && disableUPICollect) {
                PaymentMode upiCollectPaymentMode = new PaymentMode(TheiaConstant.ExtraConstants.UPI);
                List<String> upiChannels = new ArrayList<>();
                upiChannels.add(ExtraConstants.UPI);
    ...
    private void disableUPICollectForNonLoggedIn(InitiateTransactionRequestBody orderDetail) {
        ...
        if (disableUPICollect && StringUtils.isBlank(orderDetail.getPaytmSsoToken())) {
            PaymentMode upiCollectPaymentMode = new PaymentMode(ExtraConstants.UPI);
            List<String> upiChannels = new ArrayList<>();
            upiChannels.add(ExtraConstants.UPI);
            upiCollectPaymentMode.setChannels(upiChannels);
    ...
    private void mergeTLSDisabledPayModes(NativeCashierInfoRequest request) {
        ...
        HttpServletRequest httpServletRequest = (HttpServletRequest) OfflinePaymentUtils.gethttpServletRequest();
    ...
    private boolean isUpiIntentSupported(List<PayMethod> payModes) {
        ...
        payModes.stream().filter(payMode -> payMode.getPayMethod().equals(ExtraConstants.UPI)).findFirst()
    ...
    private CashierInfoRequest generateServiceRequestData(NativeCashierInfoRequest inRequest, boolean checkLoginCookie) {
        ...
        if (StringUtils.equals(inHeader.getWorkFlow(), CHECKOUT)
            ...
            if (orderDetail != null && orderDetail.getDisablePaymentMode() != null
                ...
                payModes.stream()
                        .filter(payMode -> payMode.getMode().equals(ExtraConstants.UPI))
                        .findFirst()
                        .ifPresent(
                                paymentMode -> {
        ...
        try {
            ...
            if (nativeCashierInfoRequest.getBody().isDeepLinkRequired()
                ...
                if (response != null && response.getBody() != null && null != response.getBody().getMerchantPayOption()) {
                    ...
                    for (PayMethod payMethod : payMethodList) {
                        if (ExtraConstants.UPI.equals(payMethod.getPayMethod())) {
                            fetchDeepLink = true;
                            break;
                        }
                    }
