...
import com.paytm.pgplus.upipsphandler.model.EnvInfoRequestBean;
...
import com.paytm.pgplus.models.OfferDetail;
...
import com.paytm.pgplus.upipsphandler.model.TokenRequestHeader;
import com.paytm.pgplus.stats.util.AWSStatsDUtils;
import com.paytm.pgplus.theia.redis.ITheiaSessionRedisUtil;
import com.paytm.pgplus.theia.redis.ITheiaTransactionalRedisUtil;
import com.paytm.pgplus.upipsphandler.builder.factory.ApplyPromoPaymentOptionBuilderFactory;
import com.paytm.pgplus.upipsphandler.component.*;
import com.paytm.pgplus.upipsphandler.constants.ProcessTransactionConstant;
import com.paytm.pgplus.upipsphandler.constants.PropertyConstrant;
import com.paytm.pgplus.upipsphandler.constants.ResponseCodeConstant;
import com.paytm.pgplus.upipsphandler.constants.TheiaConstant;
import com.paytm.pgplus.upipsphandler.constants.TheiaConstant.*;
import com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams;
import com.paytm.pgplus.upipsphandler.enums.EmiType;
import com.paytm.pgplus.upipsphandler.enums.RedemptionType;
import com.paytm.pgplus.pgproxycommon.enums.ResponseConstants;
import com.paytm.pgplus.upipsphandler.enums.TokenType;
import com.paytm.pgplus.upipsphandler.exception.*;
import com.paytm.pgplus.upipsphandler.model.*;
import com.paytm.pgplus.common.model.DccPageData;
import com.paytm.pgplus.upipsphandler.model.PaymentDetails;
import com.paytm.pgplus.upipsphandler.model.TransactionInfo;
import com.paytm.pgplus.upipsphandler.model.common.PayOption;
import com.paytm.pgplus.upipsphandler.service.*;
import com.paytm.pgplus.upipsphandler.utils.*;
import com.paytm.pgplus.upipsphandler.utils.EventUtils;
import com.paytm.pgplus.upipsphandler.validation.TheiaRequestValidator;
...
import com.paytm.pgplus.upipsphandler.constants.TheiaConstant.MerchantExtendedInfo.*;
import com.paytm.pgplus.common.enums.ERequestType;
import com.paytm.pgplus.upipsphandler.enums.ETransType;
...
import static com.paytm.pgplus.payloadvault.theia.constant.TheiaConstant.RequestParams.LATITUDE;
import static com.paytm.pgplus.payloadvault.theia.constant.TheiaConstant.RequestParams.LONGITUDE;
import static com.paytm.pgplus.pgproxycommon.enums.CardScheme.AMEX;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.AdditionalCallBackParam.LOGO_URL;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.EmiSubvention.VALIDATE_EMI_REQUEST;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.ExtraConstants.*;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.FF4J.*;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.FF4J.ENABLE_TPV_FOR_ALL_REQUEST_TYPES;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.LinkBasedParams.*;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestHeaders.REFERER;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestHeaders.USER_AGENT;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.*;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.CALLBACK_URL;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.Native.APP_VERSION;
...
//    private PG2Utilities pg2Utilities;
    ...
    public WorkFlowRequestBean mapWorkFlowRequestData(final PaymentRequestBean requestData)
        ...
        if (TheiaConstant.RequestTypes.ADD_MONEY.equals(requestData.getRequestType())
            ...
            flowRequestBean.setTransType(com.paytm.pgplus.common.enums.ETransType.TOP_UP);
        } else {
            flowRequestBean.setTransType(com.paytm.pgplus.common.enums.ETransType.ACQUIRING);
        }
        ...
        if (StringUtils.equals(TheiaConstant.BasicPayOption.UPI, flowRequestBean.getPayMethod()) && requestData.isDeepLinkRequired()) {
    ...
    private void setPassThroughExtendInfo(WorkFlowRequestBean flowRequestBean) {
        if (flowRequestBean.isCoftTokenTxn()) {
            ...
            if (RequestParams.DINERS.equals(flowRequestBean.getCardScheme())) {
                passThroughChannelExtendedInfo.put(TheiaConstant.RequestParams.TOKEN_UNIQUE_REFERENCE, flowRequestBean
                        .getCardTokenInfo().getTokenUniqueReference());
                passThroughChannelExtendedInfo.put(TheiaConstant.RequestParams.TOKEN_REQUESTOR_ID, flowRequestBean.getCardTokenInfo()
                        .getMerchantTokenRequestorId());
            } else {
                passThroughChannelExtendedInfo.put(TheiaConstant.RequestParams.TAVV, flowRequestBean.getCardTokenInfo().getTAVV());
            }
    ...
    private void setWalletPostPaidAuthorizationMode(WorkFlowRequestBean workFlowRequestBean,
        ...
        if (PaymentTypeIdEnum.PPI.value.equals(requestData.getPaymentTypeId())
            ...
            if (StringUtils.isNotBlank(txnToken)) {
                ...
                com.paytm.pgplus.request.InitiateTransactionRequestBody orderDetail = nativeSessionUtil.getOrderDetail(txnToken);
                if (orderDetail != null) {
    ...
    private PaymentOffer getPaymentOffersForSimplifiedOffers(WorkFlowRequestBean requestBean, String txnAmount,
        ...
        try {
            ...
            promoRequest.setHead(new com.paytm.pgplus.upipsphandler.model.TokenRequestHeader());
    ...
    private void buildEmiSubventionCheckoutReqData(WorkFlowRequestBean requestBean,
        ...
        ValidateResponse validateResponseObject = (ValidateResponse) httpServletRequest
                .getAttribute(EmiSubvention.VALIDATE_EMI_RESPONSE);
...
//    }
    ...
    private void setRiskExtendedInfoNative(WorkFlowRequestBean flowRequestBean, PaymentRequestBean requestData) {
        ...
        if (!(flowRequestBean.isScanAndPayFlow()) && StringUtils.isNotBlank(requestData.getTxnToken())) {
            com.paytm.pgplus.request.InitiateTransactionRequestBody orderDetail = nativeSessionUtil.getOrderDetail(requestData.getTxnToken());
            // setting riskExtendInfo for khatabook
    ...
    private void setExtendInfoForAdvanceDeposit(PaymentRequestBean requestData,
        ...
        try {
            ChannelAccount channelAccount = nativeSessionUtil
                    .getAdvanceDepositDetails(requestData.getTxnToken());
...
//    }
        ...
        if ((ERequestType.NATIVE.getType().equals(requestData.getRequestType())
            ...
            if (null != cashierInfoResponse) {
                try {
                    ...
                    com.paytm.pgplus.upipsphandler.model.common.ResultInfo resultInfo = new com.paytm.pgplus.upipsphandler.model.common.ResultInfo();
        ...
        if (flowRequestBean.getPaymentTypeId().equals(PaymentTypeIdEnum.NB.value)) {
            ...
            if (StringUtils.isBlank(cardTokenInfo.getTokenExpiry())
                ...
                throw new NativeFlowException.ExceptionBuilder(ResultCode.REQUEST_PARAMS_VALIDATION_EXCEPTION)
                        ...
                        .setRetryMsg(com.paytm.pgplus.upipsphandler.enums.offline.ResultCode.REQUEST_PARAMS_VALIDATION_EXCEPTION.getResultMsg()).build();
            }
        } else {
            try {
                validationService.validateAndProcessSeamlessPaymentDetails(flowRequestBean.getPaymentDetails(),
    ...
    private String setProductCode(PaymentRequestBean requestData, final WorkFlowRequestBean flowRequestBean) {
        ...
        if (flowRequestBean.isDealsFlow())
            return com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayDealAcquiringProd.getId();

        if (requestData.isAutoRefund()) {
            productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayPennyDropProd.getId();
            return productCode;
        }
        if (merchantPreferenceProvider.isSlabBasedMDREnabled(requestData)) {
            productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardAcquiringProdByAmountChargePayer.getId();
            ...
            if (workFlowHelper.getProductCodeForDynamicChargePayment(requestData.getProductCode()) == null) {
                productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayDynamicTargetProd.getId();
            } else {
            ...
            if (ERequestType.PAYTM_EXPRESS.getType().equals(requestData.getRequestType())) {
                if (ff4JUtils.isFeatureEnabledOnMid(flowRequestBean.getPaytmMID(),
                    ...
                    productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayAcquiringProdChargePayer.getId();
                } else {
                    productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardAcquiringProdByJSChargePayer.getId();
                }
            } else if (ERequestType.DYNAMIC_QR_2FA.getType().equals(requestData.getRequestType())) {
                productCode = com.paytm.pgplus.facade.enums.ProductCodes.ScanNPayChargePayer.getId();
            } else {
                productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayAcquiringProdChargePayer.getId();
            }
            ...
            productCode = com.paytm.pgplus.facade.enums.ProductCodes.QrAcquiringProd.getId();
            flowRequestBean.setPrnEnabled(true);
        } else if (requestData.isNativeAddMoney()) {
            productCode = com.paytm.pgplus.facade.enums.ProductCodes.UserTopupMultiPay.getId();
            ...
            if (StringUtils.isNotBlank(requestData.getLinkDetailsData().getEdcEmiFields().getProductCode())) {
                try {
                    productCode = com.paytm.pgplus.facade.enums.ProductCodes.getProductById(
            ...
            if (StringUtils.isBlank(productCode)) {
                productCode = com.paytm.pgplus.facade.enums.ProductCodes.StandardDirectPayAcquiringProd.getId();
            }
    ...
    private void setLinkPaymentRiskExtendedInfo(PaymentRequestBean requestData, WorkFlowRequestBean flowRequestBean) {
        if (StringUtils.isNotEmpty(requestData.getInvoiceId()) || StringUtils.isNotEmpty(requestData.getLinkId())) {
            ...
            com.paytm.pgplus.common.model.link.LinkPaymentRiskInfo linkPaymentRiskInfo = null;
            HttpServletRequest httpServletRequest = (HttpServletRequest) ((ServletRequestAttributes) RequestContextHolder
                ...
                if (splitSettlementInfo.getSplitInfo() != null) {
                    ...
                    for (SplitInfoData splitInfo : splitSettlementInfo.getSplitInfo()) {
                        ...
                        List<Goods> goodsList = SplitSettlementHelper.createGoodsListFromGoodsInfoList(splitInfo
                                .getGoods());
                        List<com.paytm.pgplus.facade.acquiring.models.ShippingInfo> shippingInfosList = SplitSettlementHelper
                        ...
                        if (SplitMethod.AMOUNT.equals(splitSettlementInfo.getSplitMethod())) {
                            ...
                            money = new com.paytm.pgplus.facade.common.model.Money(splitInfo.getAmount().getCurrency().getCurrency(),
                                    AmountUtils.getTransactionAmountInPaise(splitInfo.getAmount().getValue()));
                        }
    ...
    public String getProductCodeForRequestType(String requestType, String productCode) {
        ...
        try {
            productCode = com.paytm.pgplus.facade.enums.ProductCodes.getProductByProductCode(eRequestType.getProductCode()).getId();
        } catch (FacadeInvalidParameterException e) {
    ...
    private void checktokenIndexNumberOrCardToken(WorkFlowRequestBean flowRequestBean, PaymentRequestBean requestData)
        ...
        if (flowRequestBean.getPaymentDetails() != null) {
            ...
            if (paymentDetailsArrayLength == 2) {
                com.paytm.pgplus.facade.coft.model.UserInfo userInfo = new UserInfo();
                GenericCoreResponseBean<UserDetailsBiz> userDetails = null;
    ...
    private void setUltimateBeneficiaryDetails(WorkFlowRequestBean flowRequestBean, PaymentRequestBean requestData) {
        if (ObjectUtils.notEqual(requestData.getUltimateBeneficiaryDetails(), null)) {
            ...
            if (ObjectUtils.notEqual(initiateRequest, null)
                ...
                com.paytm.pgplus.facade.acquiring.models.UltimateBeneficiaryDetails ultimateBeneficiaryDetails = new UltimateBeneficiaryDetails();
