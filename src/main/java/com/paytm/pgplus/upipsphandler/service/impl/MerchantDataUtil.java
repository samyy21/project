...
import com.paytm.pgplus.mappingserviceclient.service.impl.BankInfoDataServiceImpl;
import com.paytm.pgplus.mappingserviceclient.service.impl.LookupDataInfoServiceImpl;
...
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.FF4J.FEATURE_THEIA_APP_INVOKE_AS_COLLECT;
...
public class MerchantDataUtil {
    ...
    private ILookupDataInfoService lookupDataServiceImpl = new LookupDataInfoServiceImpl();


    private IBankInfoDataService bankInfoDataServiceImpl = new BankInfoDataServiceImpl();
...
//    }
    ...
    public void mapMerchantDataForMPINGenerationInNative(CashierInfoRequest request,
        ...
        List<PayMethod> addMoneypayMethod = null;
        ...
        List<PayMethod> merchantPayMethod = cashierInfoResponse
    ...
    private boolean checkForUpiSpecificChannelInNative(
            List<PayMethod> payMethods, String upiPayChannel) {
        if (payMethods == null || payMethods.isEmpty()) {
            return false;
        }
        Iterator<PayMethod> payMethodIterator = payMethods
                .iterator();
        while (payMethodIterator.hasNext()) {
            PayMethod payMethod = payMethodIterator.next();
            if (payMethod.getPayChannelOptions() != null) {
                Iterator<PayChannelBase> payChannelBaseIterator = payMethod
                        .getPayChannelOptions().iterator();
                while (payChannelBaseIterator.hasNext()) {
                    PayChannelBase payChannelBase = payChannelBaseIterator
                            .next();
                    if (upiPayChannel.equals(payChannelBase.getPayChannelOption())
                            && payChannelBase.getIsDisabled() != null
                            && !Boolean.parseBoolean(payChannelBase.getIsDisabled().getStatus())) {
