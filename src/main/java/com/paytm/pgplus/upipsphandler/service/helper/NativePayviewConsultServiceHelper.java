...
import com.paytm.pgplus.upipsphandler.enums.PromoCodeType;
...
import com.paytm.pgplus.upipsphandler.service.impl.BizRequestResponseMapperHelper;
...
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.MerchantPreference.PreferenceKeys.MAX_DEFICIT_AMOUNT;
import static com.paytm.pgplus.upipsphandler.enums.EPayMethod.CREDIT_CARD;
...
//    private NpciHealthUtil npciHealthUtil;
    ...
    public NativeCashierInfoResponse transformResponse(WorkFlowResponseBean serviceResponse,
            com.paytm.pgplus.upipsphandler.model.CashierInfoRequest serviceRequest, NativeCashierInfoRequest request) {
    ...
    public void trimByTopNBChannels(NativeCashierInfoResponse response) {
        List<PayMethod> merchantPayMethods = response.getBody()
    ...
    private void trimNB(List<PayMethod> merchantPayMethods) {
        if (null != merchantPayMethods && !merchantPayMethods.isEmpty()) {
            ...
            for (PayMethod payMethod : merchantPayMethods) {
    ...
    private void trimEmiChannel(List<PayMethod> payMethods) {
        if (payMethods != null) {
...
//        List<PayMethod> merchantPayMethods = response.getBody()
//                .getMerchantPayOption().getPayMethods();
//        List<PayMethod> addMoneyPayMethods = response.getBody()
